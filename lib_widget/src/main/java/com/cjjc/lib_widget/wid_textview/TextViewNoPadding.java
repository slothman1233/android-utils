package com.cjjc.lib_widget.wid_textview;
...
public class TextViewNoPadding extends AppCompatTextView {
    //日志标记
    private final String TAG = TextViewNoPadding.class.getSimpleName();
    //文本画笔
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (_layout != null) {
            ...
            //获得行高
            int _lineHeight = (-rect.top + rect.bottom) == 0 ? (int) ((getTextSize() * getMinLines()) + getLineSpacingExtra()) : (-rect.top + rect.bottom);
            //初始化布局
            initLayout(_layout);
            //设置布局区域
            area = getWidthAndHeigt(widthMeasureSpec, heightMeasureSpec, layoutWidth, _layout.getLineCount(), _lineHeight);
            //设置布局
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (lineContents.length == 1) {
            float y = -rect.top;
            float x=(area[0]/2)-(textPaint.measureText(lineContents[0])/2);
            canvas.drawText(lineContents[0], x, y, textPaint);
        } else {
            //循环获取每行数据内容
            for (int i = 0; i < lineContents.length; i++) {
                //获得数据
                String _drawContent = lineContents[i];
                float y = -rect.top + (_line_height + _line_space) * i;
                float currentWidth = 0f;
                if (i == 0) {
                    firstLength = _drawContent.length();
                    firstWidth = textPaint.measureText(_drawContent);
                } else {
                    currentWidth = textPaint.measureText(_drawContent);
                }
                if (lineContents.length > 1 && i == lineContents.length - 1) {
                    if (currentWidth >= firstWidth) {
                        _drawContent = _drawContent.substring(0, firstLength - 2);
                        _drawContent = _drawContent + "...  ";
                    }
                }
                //绘制每行数据
                canvas.drawText(_drawContent, 0, y, textPaint);
            }
        }
    }
}
